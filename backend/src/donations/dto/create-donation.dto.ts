...
import { IsNotEmpty, IsEnum, ValidateIf, IsNumber, IsPositive, IsString, Length } from "class-validator";
...
export class CreateDonationDto {
    ...
    @IsEnum(DonationType, { message: 'Donation type must be either MONETARY or MATERIAL' })
    ...
    @ValidateIf(o => o.donationType === DonationType.MONETARY)
    @IsNotEmpty({ message: 'Monetary currency is required for monetary donations' })
    @IsString({ message: 'Monetary currency must be a string' })
    @Length(3, 3, { message: 'Monetary currency must be a valid 3-letter currency code' })
    monetaryCurrency?: string;

    // Campos para doações materiais

    @ValidateIf(o => o.donationType === DonationType.MATERIAL)
    @IsNotEmpty({ message: 'Material description is required for material donations' })
    @IsString({ message: 'Material description must be a string' })
    materialDescription?: string;

    @ValidateIf(o => o.donationType === DonationType.MATERIAL)
    @IsNotEmpty({ message: 'Material quantity is required for material donations' })
